...
Definition shiftLL (n: N) (s : N) : N := Stdlib.NArith.BinNat.N.shiftl n s.
Definition shiftRL (n: N) (s : N) : N := Stdlib.NArith.BinNat.N.shiftr n s.

Definition bit_N s := shiftLL 1%N (Stdlib.ZArith.BinInt.Z.to_N s).
...
Instance Bits__N : Bits N :=  {
  ...
  bit := bit_N;
  bitSizeMaybe := fun _ => None ;
  clearBit := fun n i => N.clearbit n (Stdlib.ZArith.BinInt.Z.to_N i) ;
  complement := fun _ => 0%N  ; (* Not legally possible with N *)
  complementBit := fun x i => N.lxor x (bit_N i) ;
  isSigned := fun x => true ;
  popCount := fun n => Z.of_N (N_popcount n);
  rotate  := fun n s => Stdlib.NArith.BinNat.N.shiftl n (Stdlib.ZArith.BinInt.Z.to_N s);
  rotateL := fun n s => Stdlib.NArith.BinNat.N.shiftl n (Stdlib.ZArith.BinInt.Z.to_N s);
  rotateR := fun n s => Stdlib.NArith.BinNat.N.shiftr n (Stdlib.ZArith.BinInt.Z.to_N s);
  setBit := fun x i => N.lor x (bit_N i);
  shift  := fun n s => Stdlib.NArith.BinNat.N.shiftl n (Stdlib.ZArith.BinInt.Z.to_N s);
  shiftL := fun n s => Stdlib.NArith.BinNat.N.shiftl n (Stdlib.ZArith.BinInt.Z.to_N s);
  shiftR := fun n s => Stdlib.NArith.BinNat.N.shiftr n (Stdlib.ZArith.BinInt.Z.to_N s);
  testBit := fun x i => N.testbit x (Stdlib.ZArith.BinInt.Z.to_N i);
  unsafeShiftL := fun n s => Stdlib.NArith.BinNat.N.shiftl n (Stdlib.ZArith.BinInt.Z.to_N s);
  unsafeShiftR := fun n s => Stdlib.NArith.BinNat.N.shiftr n (Stdlib.ZArith.BinInt.Z.to_N s);
  xor := N.lxor;
  zeroBits := 0%N;
}.
