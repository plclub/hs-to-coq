...
#[global] Definition isInteresting : Core.Var -> bool :=
  fun v => CoreArity.typeArity (Id.idType v) GHC.Base.> #0.

#[global] Definition interestingBinds : Core.CoreBind -> list Core.Var :=
  GHC.List.filter isInteresting GHC.Base.∘ Core.bindersOf.

#[global] Definition addInterestingBinds
   : Core.VarSet -> Core.CoreBind -> Core.VarSet :=
  fun int bind =>
    Core.extendVarSetList (Core.delVarSetList int (Core.bindersOf bind))
                          (interestingBinds bind).

#[global] Definition boringBinds : Core.CoreBind -> Core.VarSet :=
  Core.mkVarSet GHC.Base.∘
  (GHC.List.filter (negb GHC.Base.∘ isInteresting) GHC.Base.∘ Core.bindersOf).

#[global] Definition addCrossCoCalls
   : UnVarGraph.UnVarSet -> UnVarGraph.UnVarSet -> CallArityRes -> CallArityRes :=
  fun set1 set2 =>
    Control.Arrow.first (UnVarGraph.unionUnVarGraph
...
#[global] Definition lubRes : CallArityRes -> CallArityRes -> CallArityRes :=
  fun arg_0__ arg_1__ =>
...
#[global] Definition both : CallArityRes -> CallArityRes -> CallArityRes :=
  fun r1 r2 => addCrossCoCalls (domRes r1) (domRes r2) (lubRes r1 r2).

#[global] Definition calledMultipleTimes : CallArityRes -> CallArityRes :=
  fun res =>
    Control.Arrow.first (GHC.Base.const (UnVarGraph.completeGraph (domRes res)))
    res.

#[global] Definition emptyArityRes : CallArityRes :=
  pair UnVarGraph.emptyUnVarGraph Core.emptyVarEnv.

#[global] Definition lubRess : list CallArityRes -> CallArityRes :=
  Data.Foldable.foldl' lubRes emptyArityRes.

#[global] Definition unitArityRes
   : Core.Var -> BasicTypes.Arity -> CallArityRes :=
...
#[global] Definition calledWith
   : CallArityRes -> Core.Var -> UnVarGraph.UnVarSet :=
  fun arg_0__ arg_1__ =>
    match arg_0__, arg_1__ with
    | pair g _, v => UnVarGraph.neighbors g v
    end.

#[global] Definition callArityRecEnv
   : bool ->
  ...
  fun any_boring ae_rhss ae_body =>
      ...
      fun '(pair v ae_rhs) =>
        ...
        let ae_before_v :=
          ...
          then lubRes (lubRess (GHC.Base.map Data.Tuple.snd (GHC.List.filter
                                                             ((GHC.Prim.rightSection _GHC.Base./=_ v) GHC.Base.∘
    ...
    let ae_new :=
      Control.Arrow.first (UnVarGraph.unionUnVarGraph cross_calls) ae_combined in
    ae_new.

#[global] Definition lookupCallArityRes
   : CallArityRes -> Core.Var -> (BasicTypes.Arity * bool)%type :=
  fun arg_0__ arg_1__ =>
    match arg_0__, arg_1__ with
    | pair g ae, v =>
        match Core.lookupVarEnv ae v with
        | Some a => pair a (negb (UnVarGraph.hasLoopAt g v))
        | None => pair #0 false
        end
    end.

#[global] Definition resDel : Core.Var -> CallArityRes -> CallArityRes :=
  fun arg_0__ arg_1__ =>
    match arg_0__, arg_1__ with
    | v, pair g ae => pair (UnVarGraph.delNode g v) (Core.delVarEnv ae v)
    end.

#[global] Definition resDelList
   : list Core.Var -> CallArityRes -> CallArityRes :=
  fun vs ae => Data.Foldable.foldl' (GHC.Base.flip resDel) ae vs.

#[global] Definition trimArity
   : Core.Id -> BasicTypes.Arity -> BasicTypes.Arity :=
  fun v a =>
    let 'pair demands result_info := Core.splitDmdSig (Id.idDmdSig v) in
    let max_arity_by_strsig :=
      if Core.isDeadEndDiv result_info : bool then Coq.Lists.List.length demands else
      a in
    let max_arity_by_type := CoreArity.typeArity (Id.idType v) in
    Data.Foldable.foldr GHC.Base.min a (cons max_arity_by_type (cons
                                              max_arity_by_strsig nil)).

#[global] Definition callArityBind
   : Core.VarSet ->
     CallArityRes ->
     Core.VarSet -> Core.CoreBind -> (CallArityRes * Core.CoreBind)%type :=
  fun arg_0__ arg_1__ arg_2__ arg_3__ =>
    ...
    | boring_vars, ae_body, int, (Core.Rec binds as b) =>
        ...
        let any_boring :=
          Data.Foldable.any (GHC.Prim.rightSection Core.elemVarSet boring_vars)
          (let cont_25__ arg_26__ := let 'pair i _ := arg_26__ in cons i nil in
           Coq.Lists.List.flat_map cont_25__ binds) in
        let fix_
          ...
          HsToCoq.DeferredFix.deferredFix1 (fun fix_
                                            (ann_binds
                                              ...
                                              let aes_old :=
                                                ...
                                                Coq.Lists.List.flat_map cont_28__ ann_binds in
...
#[global] Definition callArityAnalProgram
   : Core.CoreProgram -> Core.CoreProgram :=
  fun binds =>
    let 'pair _ binds' := callArityTopLvl nil Core.emptyVarSet binds in
    binds'.

#[global] Definition callArityRHS : Core.CoreExpr -> Core.CoreExpr :=
...
(* External variables:
     ...
     Coq.Init.Datatypes.app Coq.Lists.List.flat_map Coq.Lists.List.length Core.Alt
     Core.App Core.Case Core.Cast Core.CoreBind Core.CoreExpr Core.CoreProgram
     Core.Id Core.Lam Core.Let Core.Lit Core.Mk_Coercion Core.Mk_Type Core.Mk_Var
     Core.NonRec Core.Rec Core.Var Core.VarEnv Core.VarSet Core.bindersOf
     ...
     GHC.Base.min GHC.Base.op_z2218U__ GHC.Base.op_zeze__ GHC.Base.op_zg__
     ...
     HsToCoq.DeferredFix.deferredFix1 Id.idCallArity Id.idDmdSig Id.idType
     ...
     UnVarGraph.hasLoopAt UnVarGraph.neighbors UnVarGraph.unionUnVarGraph
     UnVarGraph.unionUnVarGraphs UnVarGraph.unionUnVarSets Util.lengthExceeds
*)
