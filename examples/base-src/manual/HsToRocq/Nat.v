...
#[global]
Instance Default_nat : HsToRocq.Err.Default nat := 
  HsToRocq.Err.Build_Default _ 0.

(* Question: should we replace the - instance with this 
   checked subtraction? *)
Definition error_sub := 
  fun x y => if Nat.ltb x y then HsToRocq.Err.default else Nat.sub x y.
...
Definition enumFrom_nat : nat -> list nat := HsToRocq.Err.default.
