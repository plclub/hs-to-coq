...
Fixpoint halve {a : Type} (arg_0__ : list a) : (list a * list a)%type
  := match arg_0__ with
     | nil => pair nil nil
     | cons z zs => let 'pair xs ys := halve zs in pair (cons z ys) xs
     end.

Program Fixpoint riffle {m : Type -> Type} {a : Type}
                        `{Control.Monad.Random.Class.MonadRandom m} (arg_0__ : list a) (arg_1__
                          : GHC.Num.Int) (arg_2__ : list a) (arg_3__ : GHC.Num.Int) {measure (length
                         arg_0__ +
                         length arg_2__)} : m (list a)
...
Program Fixpoint shuffleLength {m : Type -> Type} {a : Type}
                               `{Control.Monad.Random.Class.MonadRandom m} (arg_0__ : list a) {measure (length
                                arg_0__)} : m (list a * GHC.Num.Int)%type
  := match arg_0__ with
...
Definition shuffle {m : Type -> Type} {a : Type}
  `{Control.Monad.Random.Class.MonadRandom m}
   : list a -> m (list a) :=
  GHC.Base.fmap Data.Tuple.fst GHC.Base.∘ shuffleLength.

(* External variables:
     Bool.Sumbool.sumbool_of_bool S Type cons length list nil op_ze__ op_zp__
     op_zszr__ op_zt__ pair Control.Monad.Random.Class.MonadRandom
