...
import HsToRocq.Util.List (ordNub)
import HsToRocq.Util.Has
import HsToRocq.Util.GHC.Module (ModuleName(), mkModuleNameT)

import HsToRocq.Rocq.Gallina
import HsToRocq.Rocq.Gallina.Util
import HsToRocq.Rocq.Gallina.Orphans ()
import HsToRocq.Rocq.Pretty (showP)

import HsToRocq.Edits.Types
import HsToRocq.Edits.Lexer
import HsToRocq.Edits.ParserState
}
...
RocqDefinitionRaw :: { RocqDefinition }
  : Inductive                { RocqInductiveDef  $1 }
  | Definition               { RocqDefinitionDef $1 }
  | Fixpoint                 { RocqFixpointDef   $1 }
  | Instance                 { RocqInstanceDef   $1 }
  | Axiom                    { RocqAxiomDef      $1 }
  | AssertionProof           { RocqAssertionDef  $1 }

RocqDefinition :: { RocqDefinition }
  : Coq(RocqDefinitionRaw) '.'    { $1 }
...
Edit :: { Edit }
  ...
  | redefine RocqDefinition                                { RedefinitionEdit                 $2                                    }
  | add type ModuleName RocqDefinition                     { AddEdit                          $3 $4 PhaseTyCl                       }
  | add ModuleName RocqDefinition                          { AddEdit                          $2 $3 PhaseTerm                       }
...
Coq(p)
  : EnterRocqParsing p ExitRocqParsing    { $2 }
