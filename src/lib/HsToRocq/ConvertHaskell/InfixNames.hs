...
module HsToRocq.ConvertHaskell.InfixNames (
  identIsVariable,
...
infixToRocq_ :: Op -> Ident
infixToRocq_ name = "op_" <> T.pack (zEncodeString $ T.unpack name) <> "__"

-- This is code smell: Why do we return an unstructured Ident, and not a QualId?
infixToRocq :: HasCallStack => Op -> Ident
infixToRocq op = case splitModule op of
    Just (m,op) -> m <> "." <> infixToRocq_ op
    Nothing     -> infixToRocq_ op
