...
precTable =
   ...
   where mkPrecEntry sym level assoc = (sym, (fromRocqLevel level, assoc))

-- precedence for various other expression forms
arrowPrec :: Int
arrowPrec = fromRocqLevel 90    -- right associative
  -- This number was found in here:
  -- https://github.com/coq/coq/blob/master/dev/doc/translate.txt#L95

appPrec   :: Int
appPrec   = fromRocqLevel 10    -- left associative

defaultOpPrec :: Int
defaultOpPrec = fromRocqLevel 99

scopePrec :: Int
scopePrec = fromRocqLevel 8   -- postfix, a%scope

funPrec   :: Int
funPrec   = fromRocqLevel 200

forallPrec :: Int
forallPrec = fromRocqLevel 200

matchPrec :: Int
matchPrec = fromRocqLevel 200

letPrec :: Int
letPrec = fromRocqLevel 200

ifPrec  :: Int
ifPrec  = fromRocqLevel 200

fixPrec :: Int
fixPrec = fromRocqLevel 200

castPrec :: Int
castPrec = fromRocqLevel 100

fromRocqLevel :: Int -> Int
fromRocqLevel cl = 400 - 2 * cl
...
instance Gallina Term where
  ...
  -- qualidHasValidRocqOp rejects operators with chars invalid in Coq (e.g. $)
  -- so they render as z-encoded identifiers instead of broken notation.
  renderGallina' p (App2 (Qualid op) l r) | qualidIsOp op, qualidHasValidRocqOp op =
  ...
  renderGallina' p (Qualid qid)
    | qualidIsOp qid, qualidHasValidRocqOp qid = renderQPrefix qid
...
renderUnivPoly bodies = case maximum (fmap indUnivStatus bodies) of
  UnivPolyCumulative -> "#[universes(polymorphic, cumulative)]" <> line
  UnivPoly           -> "#[universes(polymorphic)]" <> line
  NotUnivPoly        -> mempty
  where indUnivStatus (IndBody _ _ _ _ us) = us
