...
from another file `Bar`, then `hs-to-rocq` needs to know some information about
 ...
 * You need to pass `--iface-dir foo/` to make `hs-to-rocq` search for interface
 ...
 * When it cannot find an interface file, `hs-to-rocq` complains loudly (but still
 ...
 * `hs-to-rocq` can help with figuring out the right dependency order. If you pass
 ...
 * Skipping instances prevents `hs-to-rocq` from trying to load the interface
   files of the class’s module.

 * Coq types as well as information about the type classes `Eq` and `Ord` are hard-coded
   in `src/lib/HsToRocq/ConvertHaskell/BuiltIn.hs`.
