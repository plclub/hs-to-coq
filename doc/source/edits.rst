...
Effect:
  ...
  Conflicts with ``skip``, ``redefine``, and ``axiomatize`` — using
  both on the same function is an error.  ``termination ... deferred`` and
  ``termination ... corecursive`` are also incompatible (use ``deferredFix``
  instead).  ``termination ... {struct}`` is ignored with a diagnostic note
  since Equations infers structural recursion automatically.
