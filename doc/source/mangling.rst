...
to differences in parsing and keywords, ``hs-to-rocq`` must sometimes modify the
...
To name these identifiers, ``hs-to-rocq`` uses GHC's `z-encoding
...
``==``) in the output. Therefore, ``hs-to-rocq`` defines appropriate Coq syntax for it:
...
This works fine within the given module (here: ``GHC.Base``), and within every module that imports ``GHC.Base``. But in general, ``hs-to-rocq`` does not *import* modules (it only *requires* them, using ``Require GHC.Base.``), so the notation is not visible.
...
When a module needs to be required, and any of the imported names look like operators, then ``hs-to-rocq`` imports the the contained ``Notations`` module:
...
``hs-to-rocq`` include the ``ManualNotation`` module in the generated
``Notation`` module.
...
files generated by ``hs-to-rocq`` will no longer offer these notations
by default, either, but can be enabled by importing
``GHC.Base.Notation``, ``GHC.Err``, or the standard Coq
libraries. ``hs-to-rocq`` currently cannot automatically import these
