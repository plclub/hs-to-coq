...
Un-axiomatized (with concrete definitions in midamble): `substExpr`/`substBind` (mutually recursive Fixpoint), `exitifyRec` (concrete with `deferredFix2`), `Subst` type (structural axioms + `Mk_Subst` constructor), `Id.idJoinPointHood` (concrete definition). Key proven theorems: `WellScoped_substExpr` (Qed, 0 Admitted in CoreSubst.v), `exitifyProgram_WellScoped_JPV` (Qed). Still axiomatized: `cseExpr`, `cseBind`, `try_for_cse`, `floatExpr`/`floatBind`/`floatRhs`, `fiExpr`/`fiBinds`/`fiRhs`.
