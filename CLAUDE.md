...
- **`foldr' = foldr` in instances**: GHC 9.10's `envFor` renaming doesn't fire because GHC.Internal.* module names don't match the post-rename names in the renaming table. Use `redefine` for Dual/Sum/Product/Identity `foldr'`.
- **`envFor` cross-type method references**: `envFor` renames ALL class methods to local instance names, but parameterized instances (like `Foldable (Compose f g)`) reference methods for *component* types. `redefine` affected methods (Compose fold/null/length/product/sum) to use `Data.Foldable.foldMap` directly.
