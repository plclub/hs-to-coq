...
Containers v0.7. `make clean` removes `lib/` and `hs-spec/`; `make` regenerates and builds everything. IntSet/Set/Map have native v0.7 split/fromAscList/fromDescList definitions. `hs-spec/IntSetProperties.v` auto-generated from v0.7 test sources. Full type class laws verified: EqLaws, OrdLaws, SemigroupLaws, MonoidLaws, FunctorLaws for Map (all Qed in TypeclassProofs.v). Map fromList proofs fully verified (0 Admitted in FromListProofs.v). IntMapProofs.v: 3 Admitted (isSubmapOfBy_Desc/Desc1 need reformulation after isSubmapOfBy_Bin disproved; restrictKeys_Desc has 1 Bin/Tip admit) + 2 Abort with formal counterexamples (mapKeys_Desc_false, isSubmapOfBy_Bin_false). Proved: lookup_Desc0, member_Desc0, sem_Desc, goL_Desc, lookupMin_Desc, bin_Desc0, filter_Desc, delete_Desc, restrictKeys_Desc (24/25 cases). SetProofs.v: 0 Admitted (deleteMin_spec fully proved). IntSetPropertyProofs.v: 2 Admitted (thm_isProperSubsetOf/isSubsetOf — typeclass loop with Require SetProofs in make) + 1 Abort (thm_MaskPow2 unprovable). IntSetWordProofs.v: ~73 Admitted (pre-existing).
